...
namespace Seagull.Core.Data.Repository
{
    ...
    {
        public UserRoleMapRepository(LibraryDbContext context) : base(context)
        {


        }
        //public UserRoleMap GetById(int id)
        //{
        //    return _context.Set<UserRoleMap>().Where(d => d.Id == id).FirstOrDefault();
        //}
        public PagingList<UserRoleMap> GetAll(pagination pagination, sort sort, string search, string search_operator, string filter)
        {
            dynamic searchFilter = string.Empty;
            var operater = string.IsNullOrEmpty(search_operator) ? null : JObject.Parse(search_operator);
            IQueryable<UserRoleMap> query = _context.Set<UserRoleMap>();
                    ...
                    if (result.Count() > 1)
                    {
                        ...
                        result.ForEach(s =>
                        {
                            ...
                            switch ((string)_filter.Key)
                            {
                                ...
                                default:
                                    var tempQuery = query;
                                    query = count == 0 ? query.Where<UserRoleMap>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)) :
                                        query.Concat<UserRoleMap>(tempQuery.Where<UserRoleMap>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)));
                                    count = count + 1;
                                    break;
            ...
            query = query.OrderBy<UserRoleMap>(sort.predicate, !sort.reverse ? "asc" : "");


            return new PagingList<UserRoleMap>(query, pagination.start / 10, pagination.Count == 0 ? 10 : pagination.Count);
        }
    }
}
