...
{
    public class PushNotificationRepositry : Repository<PushNotification>, IPushNotificationRepositry
    {
        public PushNotificationRepositry(LibraryDbContext context) : base(context)
        {
        }
        public PushNotification GetById(int id)
        {
            return _context.Set<PushNotification>().Where(d => d.Id == id).FirstOrDefault();
        }
        public PagingList<PushNotification> GetAll(pagination pagination, sort sort, string search, string search_operator, string filter)
        {
            dynamic searchFilter = string.Empty;
            var operater = string.IsNullOrEmpty(search_operator) ? null : JObject.Parse(search_operator);
            IQueryable<PushNotification> query = _context.Set<PushNotification>().Where(x=>x.IsDeleted != true);
            ...
            if (!string.IsNullOrEmpty(filter) && filter.Length > 2)
            {
                ...
                foreach (var _filter in searchFilter)
                {
                        ...
                        result.ForEach(s =>
                        {
                            ...
                            switch ((string)_filter.Key)
                            {
                                case "":
                                    ...
                                    break;
                                default:
                                    var tempQuery = _context.Set<PushNotification>();
                                    query = count == 0 ? query.Where<PushNotification>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)) :
                                        query.Concat<PushNotification>(tempQuery.Where<PushNotification>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)));
                                    count = count + 1;
                                    break;
                    ...
                    else if (!Object.ReferenceEquals(null, _filter.Value) && !String.IsNullOrEmpty(_filter.Value.ToString()))
                    {
                        ...
                        query = query.Where<PushNotification>(
            ...
            query = query.OrderBy<PushNotification>(sort.predicate, !sort.reverse ? "asc" : "");


            return new PagingList<PushNotification>(query, pagination.start / 10, pagination.Count == 0 ? 10 : pagination.Count);
        }
    }
}
